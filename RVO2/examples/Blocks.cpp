...
void updateVisualization(RVO::RVOSimulator *sim)
{
	...
	for (size_t i = 0; i < sim->getNumAgents(); ++i) {
		...
		log.open("log.csv",ios::app);  // ofstream log("log.csv")でも可。第2引数のtruncは同名ファイルがあった場合には以前の内容を全て消す、というオプション。
		log.seekp(1,ios_base::beg);
		log<<sim->getAgentPosition(i).x()<<",";
		log<<sim->getAgentPosition(i).y()<<",";
		log.close();
		

	}
	log.open("log.csv",ios::app);
	log<<endl;
	log.close();
